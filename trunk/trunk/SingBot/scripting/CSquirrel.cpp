...
void printfunc(HSQUIRRELVM vm, const char *s,...)
{
	...
	args.push(tmp);
	g_pRootEntity->CallEvent("print", &args);
}
...
void errorfunc(HSQUIRRELVM vm, const char *s, ...)
{
	...
	arguments.push(tmp);
	g_pRootEntity->CallEvent("scriptError", &arguments);
}
/************************************/

extern CRootEntity* g_pRootEntity;

CSquirrel::CSquirrel(CResource* pResource, bool bSecure)
{
	...
	if( m_pVM )
	{
		...
		CBotNatives::LoadFunctions( this );
		CIRCNatives::LoadFunctions( this );

		// Load modules stuff
		CModules::ScriptLoad( m_pVM );
	}
}
...
CSquirrel::~CSquirrel()
{
	if(m_pVM)
	{
		// Unload script for all modules
		CModules::ScriptUnload( m_pVM );
