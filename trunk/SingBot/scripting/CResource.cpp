...
bool CResource::Reload()
{
	...
	{
		printf("Unable to load resource %s - no meta.xml\n", GetName().Get());
		return false;
	}
	...
	String strSecure;
	if(pXML->findNode("security"))
	{
		strSecure = pXML->nodeContent();
	}
...
bool CResource::Start()
{
	...
	m_pVM = new CSquirrel(this, m_bSecure);
	if( !m_pVM )
	{
		printf("[%s] Failed to create Squirrel VM.\n", GetName().Get());
		return false;
	}
	...
	if( !m_pTimers )
	{
		printf("[%s] Failed to create timers.\n", GetName().Get());
		return false;
	}

	// show a message
	printf("[%s] Started resource.\n", GetName().Get());
...
bool CResource::Stop()
{
	...
	// Show a message
	printf("[%s] Stopped Resource.", GetName().Get());
	m_eState = STATE_LOADED;
	return true;
}
